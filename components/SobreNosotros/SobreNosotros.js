    ...
    <div className="trabajadores-container">
      {trabajadores.map((trabajador, index) => (
        ...
        <div
          ...
          className={`trabajador ${index % 2 === 0 ? 'left' : 'right'}`}
          key={trabajador.nombre}
        >   
        
          <img src={trabajador.img} alt={trabajador.nombre} className="trabajador-img fancy-border-radius" />
          
          <div className="trabajador-info">
            <h2 className=' font-bold trabajador-nombre color-gris'>{trabajador.nombre}</h2>
            <hr className='linea-trabajador'/>
            <ul className='py-3 '>
              {trabajador.lista.map((item, idx) => (
                <li className=' py-3 text-xl lista-trabajador' key={idx}> {item}</li>
              ))}
            </ul>
